// xhspcv10.spec.js · 小红书PC完整主系统 V2.0补强 · TL真浏览器回归
// 运行:npx playwright test xhspcv10.spec.js
// URL:改为 https://infinity-os-sh.github.io/Infinity-os-SH/xhspcv10.html 或本地 file://
const { test, expect } = require('@playwright/test');
const URL = process.env.XURL || 'https://infinity-os-sh.github.io/Infinity-os-SH/xhspcv10.html';
...
test('V0.4 完整Loop:一条链从定标跑到下一周期(有效分支)', async ({page}) => {
  await go(page,'?role=mgr'); await page.evaluate(()=>setTab('loop')); await page.waitForTimeout(300);
  await expect(page.locator('#scroll')).toContainText('完整经营 Loop');
  await page.evaluate(()=>{ S.calib=true; });
  const HAPPY=['calib','gap','diag','decide','choose','approveDec','produce','task','task','task','task','publish','startWin','win24','win72ok','adjud','learn','next','confirmNext'];
  for(const a of HAPPY){ await page.evaluate((x)=>loopStep(x), a); await page.waitForTimeout(90); }
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].result)).toBe('有效');
  expect(await page.evaluate(()=>XPC2DBG.NEXT_CYCLES.length)).toBeGreaterThan(0);
  expect(await page.evaluate(()=>XPC2DBG.LOOPS[0].loop_status)).toBe('本周期关闭');
  // 刷新持久化
  await page.reload(); await page.waitForTimeout(600);
  expect(await page.evaluate(()=>XPC2DBG.NEXT_CYCLES.length)).toBeGreaterThan(0);
});

test('V0.4 E2E-02 信任未过禁止放量 + E2E-04 24h好72h失效', async ({page}) => {
  await go(page,'?role=mgr'); await page.evaluate(()=>setTab('loop')); await page.waitForTimeout(300);
  // E2E-02:NPS低→聚光进禁止举措
  const forbid = await page.evaluate(()=>{ LR_META['trust.note_nps'].v=41; return decBuild('trust.save_rate').forbidden.some(f=>f.m.id==='M-SPOT'); });
  expect(forbid).toBe(true);
  // E2E-04:24h好72h失效→无效
  await page.evaluate(()=>{ S.calib=true; loopSel('trust.save_rate'); });
  const FAIL72=['calib','gap','diag','decide','choose','approveDec','produce','task','task','task','task','publish','startWin','win24','win72','adjud'];
  for(const a of FAIL72){ await page.evaluate((x)=>loopStep(x), a); await page.waitForTimeout(80); }
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].result)).toBe('无效');
  // E2E-05:无效→备选·Loop不关闭
  await page.evaluate(()=>loopStep('invalidAlt')); await page.waitForTimeout(100);
  const st = await page.evaluate(()=>XPC2DBG.LOOPS[XPC2DBG.LOOPS.length-1].loop_status);
  expect(st).not.toBe('本周期关闭');
});

test('V0.4 XLOOP-09 必需任务/发布未齐不得开窗', async ({page}) => {
  await go(page,'?role=mgr'); await page.evaluate(()=>{setTab('loop');S.calib=true;}); await page.waitForTimeout(250);
  for(const a of ['calib','gap','diag','decide','choose','approveDec','produce']){ await page.evaluate((x)=>loopStep(x),a); await page.waitForTimeout(70); }
  await page.evaluate(()=>loopStep('startWin')); await page.waitForTimeout(100);
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].status)).toBe('待启动');
  await page.evaluate(()=>{['task','task','task','task','publish','startWin'].forEach(a=>loopStep(a));}); await page.waitForTimeout(120);
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].status)).toBe('观察中');
});


test('V0.5 焊接:必需任务真写入任务台V2 + evidence真算 + 有效链', async ({page}) => {
  await go(page,'?role=mgr'); await page.evaluate(()=>{setTab('loop');S.calib=true;}); await page.waitForTimeout(300);
  const n0 = await page.evaluate(()=>XPC2DBG.TASKS2.length);
  for(const a of ['calib','gap','diag','decide','choose','approveDec','produce']){ await page.evaluate((x)=>loopStep(x),a); await page.waitForTimeout(80); }
  expect(await page.evaluate(()=>XPC2DBG.TASKS2.length)).toBe(n0+4);
  // 任务未验收→evidence false→开窗被拦
  await page.evaluate(()=>loopStep('publish')); await page.evaluate(()=>loopStep('startWin')); await page.waitForTimeout(120);
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].status)).toBe('待启动');
  for(const a of ['task','task','task','task','startWin','win24','win72ok','adjud']){ await page.evaluate((x)=>loopStep(x),a); await page.waitForTimeout(80); }
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].checks.evidence)).toBe(true);
  expect(await page.evaluate(()=>XPC2DBG.VERIFS[0].result)).toBe('有效');
});

test('V0.5 Loop审批接主管六队列 + 非主管拦截', async ({page}) => {
  await go(page,'?role=mgr'); await page.evaluate(()=>{setTab('loop');loopSel('traffic.cpti');S.calib=true;}); await page.waitForTimeout(250);
  for(const a of ['calib','gap','diag','decide','choose']){ await page.evaluate((x)=>loopStep(x),a); await page.waitForTimeout(70); }
  expect(await page.evaluate(()=>XPC2DBG.S.pending.some(p=>p.loopDec))).toBe(true);
  await page.evaluate(()=>{ const i=XPC2DBG.S.pending.findIndex(p=>p.loopDec); setTab('mgrcenter'); x2QApprove(i,true); });
  expect(await page.evaluate(()=>XPC2DBG.LOOPS[XPC2DBG.LOOPS.length-1].loop_status)).toBe('举措已批准');
});

test('V0.5 P0-1 无loop_spec=N/A冲突 + Trigger骨架', async ({page}) => {
  await go(page); const src = await page.content();
  expect(src).not.toContain('loop_spec = N/A');
  expect(src).toContain('production_trigger_spec');
  await page.evaluate(()=>setTab('loop')); await page.waitForTimeout(200);
  await expect(page.locator('#scroll')).toContainText('Trigger 事件驱动骨架');
  await expect(page.locator('#scroll')).toContainText('单主线主反馈回路');
});

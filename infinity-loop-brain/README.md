...
## 九、流程闭环增量(把五步环补成「完整闭环」)

> 对应增量规格 `把五步环补成完整闭环.md`(DS)。**全部新增、不改旧逻辑**:五步环、HALF_EVEN、黄金记录全绿。

```
定标↓ → 量实↑ → [③校准] → 算差 → 关差 → [②执行确认] → 验果 → [①回写规则] ↺
                                                          关不掉 → [④上报升一层]
```

| # | 优化 | 实现 | 状态 |
|---|---|---|---|
| ① | 验果回写让环闭合 | `LoopEngine.buildClosure()` 产 `closure`{prev_*, gap_change, verdict, learned_rule_seed};种子追加 `rule_learning_log`(只进不改,**不自动改规则**) | ✅ |
| ② | 关差↔验果补「执行确认」 | RECORD 加 `response_executed`(默认 unknown)/`execution_source`;外部回填(`response_execution` 表)。verdict 三分:done+收窄=有效、done+没缩=无效要找因果、否则=无法判定 | ✅(接口) |
| ③ | 量实↔算差补「校准」 | `LoopEngine.calibrate()` **占位**:`calibrated_actual=actualRaw`、`calibrated=false`、`distortion_flag=null`;**算差已改读 calibrated_actual**(位置留对,将来校准生效自动分离) | ✅(占位) |
| ④ | 关不掉的差→上报升一层 | `LoopEngine.buildEscalation()`:连续 N 期(`loop.escalation.consecutive-n`,默认2)同一 state(欠/欠-命门)且 gap 未收窄 → `escalation` 信号 + `escalation_log`。**只产信号不自动派** | ✅ |

新增 RECORD 字段:`distortion_flag` `response_executed` `execution_source` `closure` `escalation`(`@JsonInclude(ALWAYS)`,null 也吐)。
新增表:`response_execution`(②回填源)、`rule_learning_log`(①只追加)、`escalation_log`(④)。DDL 见 `db/schema.sql`。

### 闭环验收(增量规格验收清单)
| 验收 | 状态 | 证据 |
|---|---|---|
| [回归] 原6条黄金记录全绿 | ✅ | `LoopEngineGoldenTest` 绿 |
| [①] 福海路跑完 `rule_learning_log` 有「命门→应对→有效」种子 | ✅ | `LoopClosureTest` + `LoopSeriesIntegrationTest.闭环留痕…`(走 HTTP/DB) |
| [②] `response_executed` done/unknown → verdict 不同 | ✅ | `LoopClosureTest.优化2…` |
| [③] 算差读 `calibrated_actual`;`distortion_flag` 在;黄金记录不变 | ✅ | `LoopClosureTest.优化3…` |
| [④] 连续2期未关闭 → `escalation.triggered=true` | ✅ | `LoopClosureTest.优化4…` |

> 边界(守):①种子只入库不自动改规则;②③算法先占位(执行等外部回填、校准等数据);④只产信号不自动派。
